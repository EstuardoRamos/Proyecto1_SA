...
public class ReservaRepositorioOutputAdapter implements ReservaRepositorioPort {
    ...
    public ReservaRepositorioOutputAdapter(ReservaJpaRepository jpa, EntityManager em) {
        this.jpa = jpa;
        this.em = em;
    }
    ...
    private static ReservaDbEntity toEntity(Reserva r) {
        ...
        e.setHabitacionId(r.getHabitacionId());
        e.setClienteId(r.getClienteId());
        e.setEntrada(r.getEntrada());
        e.setSalida(r.getSalida());
    ...
    @Override
    public Optional<Reserva> porId(UUID id) {
        return jpa.findById(id).map(ReservaRepositorioOutputAdapter::toDomain);
    }

    @Override
    public Page<Reserva> porHotel(UUID hotelId, Pageable pageable) {
        return jpa.findByHotelId(hotelId, pageable).map(ReservaRepositorioOutputAdapter::toDomain);
    }

    @Override
    public boolean existeTraslape(UUID roomId, LocalDate in, LocalDate out, Set<Reserva.Estado> estados) {
        var ls = estados.stream().map(Enum::name).toList();
        return jpa.existsOverlap(roomId, in, out, ls);
    }

    @Override
    public void actualizarEstado(UUID id, Reserva.Estado estado) {
        var e = jpa.getReferenceById(id);
        e.setEstado(estado.name());
        jpa.saveAndFlush(e);    // <-- guarda y hace flush
    }
